...
import com.example.guilherme.skincancerdetector.R;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void takePhoto() {
        ...
        cameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, FileProvider.getUriForFile(this,
                "com.example.guilherme.skincancerdetector.provider", image));
    ...
    public void predict(Bitmap bitmap) {
        ...
        String predictionText = prediction[0] < 0.5 ? "Determinamos que a mancha carregada se parece "+ (float)Math.round(prediction[0]*100) + "% com um melanoma, logo acreditamos ser uma mancha benigna." : "Determinamos que a mancha carregada se parece "+ (float)Math.round(prediction[0]*100) + "% com um melanoma, logo acreditamos ser uma mancha maligna.";

        //Display the prediction
        TextView predictionValue = findViewById(R.id.prediction);
        predictionValue.setText(predictionText);

        //Get value from prediction variable, transform it to string and displays on the text field
        //String resultado = String.valueOf(prediction[0]);
        //predictionValue.setText(resultado);
    }
